...
export class GitHubAdapter implements PullRequestProvider {
  private async exec(command: string, args: string[]): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      cp.execFile(command, args, { cwd: workspaceFolder }, (error, stdout, stderr) => {
        if (error) {
  ...
  private async checkGhInstalled(): Promise<void> {
    try {
      ...
      throw new Error('The GitHub CLI (gh) is not installed or not in your PATH. Please install it from https://cli.github.com/');
    }
  }
