...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand('pullgod.viewPullRequests', async () => {
    const quickPick = vscode.window.createQuickPick<vscode.QuickPickItem & { pr: PullRequest }>();
    ...
    quickPick.onDidAccept(async () => {
      ...
      if (selected) {
        ...
        try {
          ...
          vscode.window.showInformationMessage(`Checked out PR #${selected.pr.number}`);
        } catch (error) {
          vscode.window.showErrorMessage(`Error checking out pull request: ${error}`);
        }
      }
    });
